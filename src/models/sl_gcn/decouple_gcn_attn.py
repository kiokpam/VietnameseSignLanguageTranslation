...
def conv_branch_init(conv):
    ...
    k1 = weight.size(1)
    k2 = weight.size(2)
    nn.init.normal(weight, 0, math.sqrt(2.0 / (n * k1 * k2)))
...
class unit_gcn(nn.Module):
    def __init__(
        ...
        self.DecoupleA = nn.Parameter(
            torch.tensor(
                np.reshape(A.astype(np.float32), [3, 1, num_point, num_point]),
                dtype=torch.float32,
                requires_grad=True,
            ).repeat(1, groups, 1, 1),
            requires_grad=True,
        ...
        self.Linear_bias = nn.Parameter(
            torch.zeros(
                1, out_channels * num_subset, 1, 1, requires_grad=True,
            ),
            requires_grad=True,
        )
        ...
        eyes = torch.stack(eye_array)
        eyes.requires_grad = False
        self.eyes = nn.Parameter(eyes, requires_grad=False)  # [c,25,25]
    ...
    def forward(self, x0):
        ...
        norm_learn_A = torch.cat(
            [
                self.norm(learn_A[0:1, ...]),
                self.norm(learn_A[1:2, ...]),
                self.norm(learn_A[2:3, ...]),
            ],
            0,
        )
...
class Model(nn.Module):
    def __init__(
        ...
        self.graph = Graph(num_node=num_points, labeling_mode=labeling_mode)
        A = self.graph.A
        self.data_bn = nn.BatchNorm1d(num_people * in_channels * num_points)

        self.l1 = TCN_GCN_unit(
            in_channels, 64, A, groups, num_points, block_size, residual=False
        )
        self.l2 = TCN_GCN_unit(64, 64, A, groups, num_points, block_size)
        self.l3 = TCN_GCN_unit(64, 64, A, groups, num_points, block_size)
        self.l4 = TCN_GCN_unit(64, 64, A, groups, num_points, block_size)
        self.l5 = TCN_GCN_unit(64, 128, A, groups, num_points, block_size, stride=2)
        self.l6 = TCN_GCN_unit(128, 128, A, groups, num_points, block_size)
        self.l7 = TCN_GCN_unit(128, 128, A, groups, num_points, block_size)
        self.l8 = TCN_GCN_unit(128, 256, A, groups, num_points, block_size, stride=2)
        self.l9 = TCN_GCN_unit(256, 256, A, groups, num_points, block_size)
        self.l10 = TCN_GCN_unit(256, 256, A, groups, num_points, block_size)
