...
from .attention import DepthWiseConv2d, Mlp, bn_init, conv_init
...
class unit_san(nn.Module):
    ...
    def forward(self, x0):
        ...
        attention = (
            self.tan(torch.einsum("nkctu,nkctv->nkuv", [q, k]) / (self.inter_channels * t))
            * self.alphas
        )
        ...
        x = torch.einsum("nkctv,nkcvw->nkctw", (x, self.attention0s.repeat(n, 1, 1, 1, 1))).view(
            n, -1, t, v
        )
...
class unit_tan(nn.Module):
    ...
    def forward(self, x, keep_prob, A):
        ...
        attention = (
            F.softmax(torch.einsum("nctv,nctuv->nctu", x, upfold) / v, -1) + self.attention0s
        )
...
class global_tan(nn.Module):
    def __init__(
        ...
        self.dropT = DropBlockT_1d(block_size=block_size)
        self.attention0s = nn.Parameter(
            torch.zeros(1, out_channels, window_size, window_size) + torch.eye(window_size),
            requires_grad=True,
        )
    ...
    def forward(self, x, keep_prob, A):
        ...
        attention = (
            F.softmax(torch.einsum("nctv,ncyv->ncty", x, x_global) / v, -1) + self.attention0s
        )
        # print(attention[0,0,:5,:5])
...
class Edge_feature_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.transform(x)
        attention = (
            F.tanh(torch.einsum("nctv, ce->ntev", x, self.edge_features)) / (c) * self.alphas
        )
...
class Block(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.A = nn.Parameter(
            torch.tensor(
                np.sum(np.reshape(A.astype(np.float32), [3, num_point, num_point]), axis=0),
                dtype=torch.float32,
                requires_grad=False,
            ),
            requires_grad=False,
...
class Model(nn.Module):
    def __init__(
        ...
        A = Graph(labeling_mode=labeling_mode, graph=graph).A
        ...
        self.fc = nn.Linear(inner_dim * inner_dim_expansion[-1], num_classes)
    ...
    def forward(
        self,
        x: torch.Tensor,
        keep_prob: torch.Tensor = torch.tensor([1.0]),
    ) -> torch.Tensor:
        N, C, T, V, M = x.size()
        ...
        keep_prob = keep_prob.cpu().detach().item()
